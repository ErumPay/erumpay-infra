...
CREATE TABLE IF NOT EXISTS card_registered (
  ...
  encrypted_billing_key VARCHAR(255) NULL COMMENT 'PG 발급 빌링키 암호화 저장, REGISTERING 상태에서는 NULL 허용',
  masked_number VARCHAR(20) NULL COMMENT 'REGISTERING 상태에서는 NULL 허용',
  card_alias VARCHAR(50) NULL,
  expiry_ym CHAR(6) NOT NULL COMMENT 'YYYYMM',
  is_default BOOLEAN NOT NULL DEFAULT FALSE,
  status ENUM('ACTIVE','PAUSED','EXPIRED','DELETED','REGISTERING') NOT NULL DEFAULT 'ACTIVE',
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
